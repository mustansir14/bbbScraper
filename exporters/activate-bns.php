...
$srcDb->connectOrDie(
    ...
    $config["source_db"]["pass"],
    $config["source_db"]["name"]
);
...
foreach ($companyUrls as $companyUrl) {
    ...
    $companyID = $srcDb->queryColumnRow(
        "select company_id from company where url = '".$srcDb->escape($companyUrl)."'",
        "company_id"
    );
...
foreach ($companies as $companyNbr => $companyId) {
    ...
    if (!$sourceCompanyRow) {
        die("Error: ".$srcDb->getExtendedError());
    }
    ...
    $companyBusinessID = $exporter->getCompanyImportID($sourceCompanyRow["company_id"]);
    $businessImportID = $exporter->getBusinessImportID($sourceCompanyRow["company_id"]);

    $destCompanyID = $exporter->isCompanyExists($companyBusinessID, null);
    if (!$destCompanyID) {
        echo "Skip: company not exists\n";
        continue;
    }

    $destBusinessID = $exporter->isBusinessExists($businessImportID, null);
    if (!$destBusinessID) {
        echo "Skip: business not exists\n";
        continue;
    }

    $url = $profile["host"]."/asd-b".$destBusinessID;

    /*if ($exporter->isBusinessActive($businessImportID)) {
        echo $url."\n";
        echo "Skip: business profile is active, may be already records changed!\n";
        continue;
    }*/

    if($exporter->getBusinessImportedComplaintsCount($businessImportID) == 0 && $exporter->getBusinessImportedReviewsCount($businessImportID) == 0) {
        echo $url."\n";
        echo "Skip: no reviews & complaints!\n";
        continue;
    }

    BusinessData::unspamAllPosts($exporter, $sourceCompanyRow, 'review');
    BusinessData::unspamAllPosts($exporter, $sourceCompanyRow, 'complaint');

    if(!$exporter->enableBusiness($businessImportID)) {
        echo "Enable business error: ".$exporter->getErrorsAsString()."\n";
        continue;
    }
