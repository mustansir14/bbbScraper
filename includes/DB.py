...
class DB:
    ...
    def reconnect(self):
        ...
        logging.info("DB Reconnect with " + self.user + ':********@' + self.host + ' use ' + self.db)
    ...
    def setSetting(self, name, value):
        self.execSQL('insert into `settings` set `name` = ?, `value` = ? ON DUPLICATE KEY UPDATE `value` = ?',
                     (name, value, value,))
    ...
    def move_items(self, table, fromCompanyId, toCompanyId):
        ...
        while 1:
            rows = self.queryArray("select " + table + "_id from " + table + " where company_id = ? limit 1000",
                                   (fromCompanyId,))
            if len(rows) == 0:
                break
            ...
            for row in rows:
                ...
                try:
                    self.execSQLWithoutIgnoreDuplicates(
                        "update " + table + " set company_id = ? where " + table + "_id = ?", (toCompanyId, id,))
                except Exception as e:
    ...
    def mark_company_as_success(self, url):
        self.execSQL('update company set date_updated=now(), status = "success" where url = ?', (url,))
        ...
        try:
            ...
            if row is not None:
                ...
                sql = """INSERT INTO company SET 
                    ...
                    half_scraped = ?, 
                    country = ?"""
                args = (company.name, company.alternate_business_name, company.url, company.logo, company.categories,
                        company.phone, company.address,
                        company.street_address, company.address_locality, company.address_region, company.postal_code,
                        company.website, company.hq, company.is_accredited, company.bbb_file_opened,
                        ...
                        company.referral_assistance, company.refund_and_exchange_policy, company.business_categories,
                        company.facebook, company.instagram, company.twitter, company.pinterest, company.linkedin,
                        company.source_code, company.source_code_details,
    ...
    def insert_or_update_reviews(self, reviews: List[Review]):
        ...
        for review in reviews:
            try:
                ...
                row = self.queryRow(
                    "SELECT review_id from review where company_id = ? and review_date = ? and username = ?;",
                    (review.company_id, review.review_date, review.username))
                if row is not None:
                    ...
                    sql = """UPDATE review SET 
                        ...
                        log = ? 
                    where review_id = ?"""
                    args = (review.review_text, review.review_rating, review.company_response_text,
            ...
            try:
                ...
                if row is not None:
                    ...
                    sql = """UPDATE complaint SET 
                        ...
                        log = ? 
                        where complaint_id = ?"""
                    args = (complaint.complaint_type, complaint.complaint_date, complaint.complaint_text,
                            complaint.company_response_text, complaint.company_response_date, complaint.source_code,
                            datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), complaint.status, complaint.log,
                            complaint_id)
                else:
